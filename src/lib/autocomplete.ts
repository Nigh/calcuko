...
const builtinCompletions: Completion[] = Object.entries(mathContext)
	...
	.map(([label]) => ({ label, type: "function", detail: mathFunctions[label] ?? "内置函数", boost: 0 }))
	...
	for (const line of source.slice(0, lineStart).split("\n")) {
		try {
			...
			if (statement.kind === "functionDefinition") {
				options.set(statement.name, { label: statement.name, type: "function", detail: `自定义函数 (${statement.parameters.join(", ")})`, boost: 2 });
				...
				options.set(statement.name, parameters
					? { label: statement.name, type: "function", detail: `自定义函数 (${parameters.join(", ")})`, boost: 2 }
					: { label: statement.name, type: "variable", detail: "上方变量", boost: 1 });
			} else if (statement.kind === "destructuringAssignment") {
				for (const label of statement.names) options.set(label, { label, type: "variable", detail: "上方变量", boost: 1 });
			}
		} catch {
