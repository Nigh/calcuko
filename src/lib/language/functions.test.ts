...
describe("user functions", () => {
	it("parses def and lambda forms", () => {
		expect(parse("def add(a,b)=a+b")).toMatchObject({ kind: "functionDefinition", name: "add", parameters: ["a", "b"] });
	...
	it("evaluates definitions and lambdas", () => {
		const result = evaluateSource("def square(x)=x**2\nsquare(12)\ntriple = x => x*3\ntriple(7)");
		expect(result.lineResults.map((line) => line.text)).toEqual(["square = <function square(x)>", "144", "triple = <function(x)>", "21"]);
	});
	...
	it("supports recursion and validates arity", () => {
		const result = evaluateSource("def fact(n)=n<=1?1:n*fact(n-1)\nfact(10)\nfact()");
