...
export const mathFunctions: Record<string, string> = {
	...
	len: "len(array) — 获取数组长度",
	reverse: "reverse(array) — 反转数组顺序",
	sum: "sum(array) — 数组求和",
	ave: "ave(array) — 数组平均值",
	minArray: "minArray(array) — 获取数组最小值",
	maxArray: "maxArray(array) — 获取数组最大值",
	map: "map(array, fn) — 映射数组",
	aggregate: "aggregate(array, fn) — 逐项归并数组",
	filter: "filter(array, fn) — 过滤数组",
	sort: "sort(array, fn?) — 排序数组",
	unique: "unique(array) — 数组去重",
	count1: "count1(width, value) — 统计置位比特数",
	rotateL: "rotateL(width, value, n?) — 循环左移",
	rotateR: "rotateR(width, value, n?) — 循环右移",
	reverseBits: "reverseBits(width, value) — 翻转比特顺序",
	reverseBytes: "reverseBytes(width, value) — 翻转字节顺序",
	swapNib: "swapNib(width, value) — 交换半字节",
	evenParity: "evenParity(width, value) — 计算偶校验位",
	oddParity: "oddParity(width, value) — 计算奇校验位",
	pack: "pack(bytes, values) — 将整数数组打包",
	unpack: "unpack(bytes, count, value) — 将整数拆分为数组",
	matrix: "matrix(rows) — 从二维数组构造矩阵",
	row: "row(values...) — 构造单行矩阵",
	col: "col(values...) — 构造单列矩阵",
