...
- **📐 可选量纲计算**：标题栏开关可启用受 [Numbat](https://github.com/sharkdp/numbat) 启发的高精度物理单位、量纲校验、智能简化和显式换算。
...
打开标题栏中的「量纲」开关后，可以使用带空格的单位表达式和 `->` 换算：
...
半径 = 5 m
π = PI
面积 = π * 半径**2      // 78.5398163397448 ㎡
```

加减和比较要求量纲兼容，乘除、幂、数组、矩阵、统计以及适用数学函数会传播量纲；结果优先保留有用的输入单位，并可简化为常见导出单位。开关首次使用时默认关闭，之后会恢复已保存的选择。

量纲模式中，`**` 对完整量值乘方，`^` 表示单位角标：`10 m**2` 等同 `(10 m)**2 = 100㎡`，而 `10 m^2` 表示 `10㎡`。关闭量纲模式后，`^` 仍是按位异或。`10m` 仍表示数字的 milli 后缀，十米必须写成 `10 m`。单位范围面向科学与工程，包含 SI、常用英美/航海、天文、原子/核物理和数字信息单位，不包含货币、无关计数、显示及幽默单位。
